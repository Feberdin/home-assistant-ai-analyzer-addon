...
def answer_chat_question(settings: AppSettings, question: str, reports: dict) -> str:
    """Return a local or AI-assisted answer for the dashboard chat."""

    cleaned_question = question.strip()
    if not cleaned_question:
        return "Bitte stelle eine konkrete Frage, zum Beispiel zu fehlenden Entities, teuren Templates oder einer neuen Automation."

    if settings.enable_ai and settings.llm_api_key:
        return _answer_with_ai(settings, cleaned_question, reports)

    return _answer_locally(cleaned_question, reports, ai_enabled=settings.enable_ai)


def _answer_locally(question: str, reports: dict, ai_enabled: bool) -> str:
    ...
    if not ai_enabled:
        lines.append("Hinweis: Fuer einen echten YAML-Generator im Chat aktiviere im Add-on `enable_ai` und hinterlege einen gueltigen API-Schluessel.")
...
def _answer_with_ai(settings: AppSettings, question: str, reports: dict) -> str:
